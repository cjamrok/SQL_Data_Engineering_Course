...
SELECT sd.skills, round(median(salary_year_avg), 0) as "median_salary" FROM job_postings_fact jpf
    left join skills_job_dim skz
        ON jpf.job_id = skz.job_id
    left join skills_dim sd
        ON skz.skill_id = sd.skill_id
WHERE job_title_short = 'Data Engineer' AND job_work_from_home = TRUE --AND (salary_year_avg > )
GROUP BY sd.skills
